...
const api = {
  ...
  setTabTimer: (id: number, minutes: number): Promise<void> =>
    ipcRenderer.invoke('tab:timer', id, minutes),
  safeStart: (): Promise<boolean> => ipcRenderer.invoke('health:safe-start'),
  restartBrowser: (safe: boolean): Promise<void> => ipcRenderer.invoke('health:restart', safe),
  pageFailures: (): Promise<PageFailure[]> => ipcRenderer.invoke('health:failures'),
  clearPageFailures: (): Promise<void> => ipcRenderer.invoke('health:clear-failures'),
  checkNetwork: (host = ''): Promise<NetworkCheck> => ipcRenderer.invoke('health:network', host),
  lastCrash: (): Promise<CrashReport | null> => ipcRenderer.invoke('health:crash'),
  dismissCrash: (): Promise<void> => ipcRenderer.invoke('health:dismiss-crash'),
