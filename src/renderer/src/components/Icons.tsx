...
/* --- пометки на снимке ------------------------------------------------- */
/** Стрелка как пометка: с древком и наконечником, а не угловая птичка. */
export const ArrowMark = (p: P) => (
  <svg {...svg(p, 1.7)}>
    <path d="M6 18 18 6" />
    <path d="M11 6h7v7" />
  </svg>
)
/** Рамка: пустой прямоугольник — то, что этот инструмент и рисует. */
export const FrameMark = (p: P) => (
  <svg {...svg(p, 1.7)}>
    <rect x="4" y="5.5" width="16" height="13" rx="2.5" />
  </svg>
)
/** Подпись: буква «Т», как её рисуют в любом редакторе. */
export const TextMark = (p: P) => (
  <svg {...svg(p, 1.8)}>
    <path d="M5.5 6.5h13M12 6.5v11" />
  </svg>
)
...
/**
 * Passwords and cards, which are now one place: a card with a keyhole in it.
