...
the browser's own LANGUAGES table, so the two lists can never drift apart.
  ...
  python gen-installer-languages.py            rewrite the .nsh files
  python gen-installer-languages.py --check    say whether they are current
"""
import io, os, re, sys

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
NSH = os.path.join(ROOT, 'build', 'installer.nsh')
...
LANGS_NSH = os.path.join(os.path.dirname(NSH), 'installer-langs.nsh')
...
langs_src = chr(10).join(lines) + chr(10)

if CHECK:
    # The question is only whether re-running would change anything, so the
    # BOM and the line endings are not part of the comparison.
    flat = lambda text: text.replace(chr(13) + chr(10), chr(10))
    stale = []
    if flat(io.open(NSH, encoding='utf-8-sig').read()) != src:
        stale.append('build/installer.nsh')
    if flat(io.open(LANGS_NSH, encoding='utf-8-sig').read()) != langs_src:
        stale.append('build/installer-langs.nsh')
    if stale:
        print('STALE, LANGUAGES has moved on without ' + ', '.join(stale))
        print('run: python build/tools/gen-installer-languages.py')
        sys.exit(1)
    print('installer .nsh: up to date with %d languages' % len(langs))
    sys.exit(0)

io.open(NSH, 'w', encoding='utf-8-sig', newline=chr(13) + chr(10)).write(src)
io.open(LANGS_NSH, 'w', encoding='utf-8', newline=chr(13) + chr(10)).write(langs_src)
